      ...
      <div className="p-4 space-y-4">
        {isExporting ? (
          <div className="text-center py-4 space-y-4">
            ...
            <h3 className="text-lg font-bold text-white">Exporting {exportFormat === 'video' ? 'Video' : exportFormat === 'image-sequence' ? 'Images' : 'Project'}</h3>
            ...
            <p className="text-white/50 text-xs">{exportProgress}% complete</p>
          </div>
        ) : (
          <>
            {/* Export Format */}
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <button
                    key={item.value}
                    onClick={() => setExportFormat(item.value as any)}
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <button
                    key={item.value}
                    onClick={() => setExportDestination(item.value as any)}
                    className={`flex flex-col items-center p-2 rounded-lg border text-xs ${
                      exportDestination === item.value
                        ? 'bg-amber-500/20 border-amber-500/50 text-white'
                        : 'bg-white/5 border-white/10 text-white/70 hover:bg-white/10'
                    }`}
                  >
            ...
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-2 text-xs text-white/80">
              <p>
                {exportFormat === 'video' && 'Video export may take several minutes depending on project complexity.'}
                {exportFormat === 'image-sequence' && 'Image sequence will be exported as a ZIP file.'}
                {exportFormat === 'json' && 'JSON contains project configuration for backup.'}
              </p>
            </div>
          </>
        )}
