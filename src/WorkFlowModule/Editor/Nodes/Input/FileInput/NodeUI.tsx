...
export default function FileInputNode({ data }: FileInputNodeProps) {
  const [isExpanded, setIsExpanded] = useState(!data.minimized);
  const [showConfig, setShowConfig] = useState(false);

  const config = data.config as FileInputConfig;
            ...
            <div className="mt-6 pt-4 border-t">
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={config.allowedTypes?.join(', ') || 'txt, pdf, csv, json, xml'}
                    onChange={(e) => handleConfigChange('allowedTypes', e.target.value.split(',').map(f => f.trim()))}
                    className="w-full px-2 py-1 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                </div>
              </div>
