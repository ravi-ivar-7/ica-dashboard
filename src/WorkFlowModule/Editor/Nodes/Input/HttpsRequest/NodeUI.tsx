...
import { HttpsRequestConfig } from './Types';
...
export default function HttpsRequestNode({ data }: HttpsRequestNodeProps) {
  const [isExpanded, setIsExpanded] = useState(!data.minimized);
  const [showOptionalInputs, setShowOptionalInputs] = useState(false);
  const [showConfig, setShowConfig] = useState(false);

  const config = data.config as HttpsRequestConfig;
      ...
      <div className="ml-6">
        ...
        {!input.connected && (
          <div className="mt-1">
            ...
            {input.type === 'any' && input.name === 'headers' && (
              <textarea
                value={input.value ? JSON.stringify(input.value, null, 2) : ''}
                onChange={(e) => {
                  try {
                    handleInputChange(input.id, JSON.parse(e.target.value));
                  } catch (error) {
                    // Handle invalid JSON input
                    console.error("Invalid JSON for headers", error);
                  }
                }}
                placeholder={input.description || `Enter ${input.name} (JSON)`}
                rows={4}
                className="w-full px-2 py-1 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none"
              />
            )}
            {input.type === 'any' && input.name === 'body' && (
              <textarea
                value={input.value ? JSON.stringify(input.value, null, 2) : ''}
                onChange={(e) => {
                  try {
                  ...
                  <textarea
                    ...
                    onChange={(e) => {
                      try {
                        ...
                        console.error("Invalid JSON for config headers", error);
                      }
                    }}
                    rows={4}
                    className="w-full px-2 py-1 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none"
