...
export default function ImageGenerationNode({ data }: ImageGenerationNodeProps) {
  const [isExpanded, setIsExpanded] = useState(!data.minimized);
  const [showOptionalInputs, setShowOptionalInputs] = useState(false);
  const [showConfig, setShowConfig] = useState(false);

  const config = data.config as ImageGenerationConfig;
          ...
          {showConfig && (
            <div className="mt-6 pt-4 border-t">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-600 block mb-1 font-medium">Model</label>
                  <select
                    value={config.model || 'SDXL'}
                ...
                <div>
                  <label className="text-xs text-gray-600 block mb-1 font-medium">Steps</label>
                  <input
                    type="number"
                    value={config.steps || 20}
                    onChange={(e) => handleConfigChange('steps', parseInt(e.target.value) || 20)}
                    min="1"
                    max="100"
                    className="w-full px-2 py-1 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                ...
                <div>
                  <label className="text-xs text-gray-600 block mb-1 font-medium">Guidance Scale</label>
                  <input
                    type="number"
                    value={config.guidance_scale || 7.5}
                    onChange={(e) => handleConfigChange('guidance_scale', parseFloat(e.target.value) || 7.5)}
                    min="1"
                    max="20"
                    step="0.5"
                    className="w-full px-2 py-1 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                ...
                <div className="col-span-2">
                  <label className="text-xs text-gray-600 block mb-1 font-medium">Seed (-1 for random)</label>
                  <input
                    type="number"
                    value={config.seed || -1}
