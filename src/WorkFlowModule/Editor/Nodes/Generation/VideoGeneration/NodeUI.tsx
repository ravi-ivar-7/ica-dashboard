...
export default function VideoGenerationNode({ data }: VideoGenerationNodeProps) {
  const [isExpanded, setIsExpanded] = useState(!data.minimized);
  const [showOptionalInputs, setShowOptionalInputs] = useState(false);
  const [showConfig, setShowConfig] = useState(false);

  const config = data.config as VideoGenerationConfig;
          ...
          <div className="mt-1">
            ...
            {input.type === 'image' && (
              <input
                ...
                placeholder={input.description || `Enter ${input.name} URL`}
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={config.fps || 24}
                    onChange={(e) => handleConfigChange('fps', parseInt(e.target.value) || 0)}
                    className="w-full px-2 py-1 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    value={config.resolution || '1280x720'}
