  ...
  useEffect(() => {
    const fetchAssets = async () => {
      ...
      try {
        ...
        interface VoiceAsset extends Asset {
          name: string;
          tags: string[];
        }

        const voiceAssets: VoiceAsset[] = mockAssets.map((asset: Asset): VoiceAsset => ({
          ...asset,
          name: asset.name.replace('Ambient_Track', 'Voice_Clip'),
          tags: [...asset.tags.filter((tag: string) => tag !== 'ambient'), 'voice', 'speech']
        }));
