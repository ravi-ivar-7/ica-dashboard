...
import { AudioModel } from '@/AudioModule/Types/audio';
...
export default function MusicGenerator() {
  ...
  const [selectedModel, setSelectedModel] = useState<AudioModel | null>(null);
  ...
  useEffect(() => {
    const fetchAssets = async () => {
      ...
      try {
        ...
        interface MusicAsset extends Asset {
          name: string;
          tags: string[];
        }

        const musicAssets: MusicAsset[] = mockAssets.map((asset: Asset): MusicAsset => ({
          ...asset,
          name: asset.name.replace('Ambient_Track', 'Music_Track'),
          tags: [...asset.tags.filter((tag: string) => tag !== 'ambient'), 'music', 'composition']
        }));
